...
/*   Updated: 2021/12/04 23:29:37 by rimartin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	ft_horizontal_line(float ra)
{
	...
	if (!tan(ra))  // Se o valor da tangente for 0, c
	{
...
static void	ft_print_one_ray_h(float rx, float ry, float ra, int color)
{
	...
	i = 0;
	printf("ry %f\n", ry);
	delta_x = fabs(ry - g_master.trigo.pixel_x); // chyba zle drukuje, trzeba popracoawc nad delta!!!, moze 2 rozne funkcje dac!!
